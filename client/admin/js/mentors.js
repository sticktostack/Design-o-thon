...
mentorForm.addEventListener("submit", async (e) => {
  ...
  const response = await fetch("http://localhost:5000/api/mentor/add", {
    method: "POST",
    headers: {
...
async function assignTeam(mentorId) {
  ...
  const response = await fetch("http://localhost:5000/api/mentor/assign-team", {
    method: "POST",
    headers: {
      "Content-Type": "application/json"
    },
...
async function deleteMentor(id) {
  await fetch(`http://localhost:5000/api/mentor/delete/${id}`, {
    method: "DELETE"
  });
